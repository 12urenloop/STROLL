...
if [[ $controller_count -gt 1 ]]; then
  echo "Multiple bluetooth controllers detected"
  echo "Select your preferred one to scan on"
  echo 
  i=1
  ...
  if [[ $choice -lt 1 || $choice -gt $controller_count ]]; then
    echo "There is no controller with that number, sorry :/"
    exit 2
  fi
...
echo "Using bluetooth controller with mac: [$BT_STATION_MAC]"
